...
static PetscErrorCode FETI2SetUpCoarseProblem_Private(FETI ft)
{
  ...
  if (ft1->n_rbm){
    ...
    ierr = VecCreateSeq(PETSC_COMM_SELF,ft1->n_rbm,&ft1->alpha_local);CHKERRQ(ierr);
    ...
    ft1->displ_f[0]      = 0;
    ft1->count_f_rbm[0]  = n_rbm_comm[0];
    k                    = (n_rbm_comm[0]>0);
    for (i=1;i<sizeG;i++){
      if(n_rbm_comm[i]) {
	ft1->count_f_rbm[k] = n_rbm_comm[i];
...
static PetscErrorCode FETI2FactorizeCoarseProblem_Private(FETI ft)
{
  ...
  PetscFunctionBegin; 
  if(!ft1->coarse_problem) SETERRQ(PetscObjectComm((PetscObject)ft),PETSC_ERR_ARG_WRONGSTATE,"Error: FETI2SetUpCoarseProblem_Private() must be first called");
...
static PetscErrorCode FETI2ApplyCoarseProblem_Private(FETI ft,Vec v,Vec r)
{
  ...
  PetscFunctionBegin;
  ierr = PetscObjectGetComm((PetscObject)ft,&comm);CHKERRQ(ierr);
  if(!ft1->F_coarse) SETERRQ(comm,PETSC_ERR_ARG_WRONGSTATE,"Error: FETI2FactorizeCoarseProblem_Private() must be first called");
  ierr = MPI_Comm_rank(comm,&rank);CHKERRQ(ierr);
...
static PetscErrorCode FETI2ComputeAlpha_Private(FETI ft){
  ...
  PetscFunctionBegin;
  ierr = VecUnAsmGetLocalVector(ft1->res_interface,&lambda_local);CHKERRQ(ierr);
  ...
  ierr = MPI_Allgatherv(sbuff,ft1->n_rbm,MPIU_SCALAR,rbuff,ft1->count_f_rbm,ft1->displ_f,MPIU_SCALAR,ft1->floatingComm);CHKERRQ(ierr);
