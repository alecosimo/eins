...
static PetscErrorCode PCApplyLocal_DIRICHLET(PC pc,Vec x,Vec y)
{
 PCFT_DIRICHLET      *pcd = (PCFT_DIRICHLET*)pc->data;
  ...
  for (i=1; i<ft->n_neigh_lb; i++){
    ierr = VecGetEntriesInArrayFromLocalVector(x,pcd->isindex[i-1],pcd->send_arrays[i-1]);CHKERRQ(ierr);
    ierr = PetscMPIIntCast(ft->neigh_lb[i],&i_mpi);CHKERRQ(ierr);   
    ierr = MPI_Isend(pcd->send_arrays[i-1],ft->n_shared_lb[i],MPIU_SCALAR,i_mpi,pcd->tag,pcd->comm,&pcd->s_reqs[i-1]);CHKERRQ(ierr);
  }
  ...
  for (i=1; i<ft->n_neigh_lb; i++){
    ...
    /* communicate result */
    ierr = VecGetEntriesInArrayFromLocalVector(vec_res,pcd->isindex[i-1],pcd->send_arrays[i-1]);CHKERRQ(ierr);
    ierr = PetscMPIIntCast(ft->neigh_lb[i],&i_mpi);CHKERRQ(ierr);   
    ierr = MPI_Isend(pcd->send_arrays[i-1],ft->n_shared_lb[i],MPIU_SCALAR,i_mpi,pcd->tag,pcd->comm,&pcd->s_reqs[i-1]);CHKERRQ(ierr);
  } 
  /* receive results */
...
static PetscErrorCode PCApplyLocalWithPolling_DIRICHLET(PC pc,Vec x,Vec y,PetscInt *_n2c)
{
  ...
  if (n2c) {
	...
	ierr = VecGetEntriesInArrayFromLocalVector(x,pcd->isindex[is],pcd->send_arrays[is]);CHKERRQ(ierr);
    	ierr = PetscMPIIntCast(ft->neigh_lb[is+1],&i_mpi);CHKERRQ(ierr);   
	ierr = MPI_Isend(pcd->send_arrays[is],ft->n_shared_lb[is+1],MPIU_SCALAR,i_mpi,pcd->tag,pcd->comm,&pcd->s_reqs[is]);CHKERRQ(ierr);
      }
    }
    /* receive vectors from my neighbors */
    ir = 0;
	...
	ierr = MatMult(ft->B_Ddelta,sd->vec2_B,vec_res);CHKERRQ(ierr);
	/* communicate result */
	ierr = VecGetEntriesInArrayFromLocalVector(vec_res,pcd->isindex[i],pcd->send_arrays[i]);CHKERRQ(ierr);
	ierr = PetscMPIIntCast(ft->neigh_lb[i+1],&i_mpi);CHKERRQ(ierr);   
	ierr = MPI_Isend(pcd->send_arrays[i],ft->n_shared_lb[i+1],MPIU_SCALAR,i_mpi,pcd->tag,pcd->comm,&pcd->s_reqs[is++]);CHKERRQ(ierr);
      }
    } 
    /* receive results */
    ir = 0;
