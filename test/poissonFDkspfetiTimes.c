...
int main(int argc,char **args)
{
  ...
  Vec                      localRHS=0,u=0,lgRHS=0;
  ISLocalToGlobalMapping   mapping=0;
  FETI                     feti;
  KSP                      ksp_interface,ksp_feti;
  PetscInt                 lsize,its;
  char                     nameFileIts[14];
  PetscMPIInt              sizeG;
  PetscBool                take_times;
  ...
  ierr = VecCreateMPIUnasmWithLocalVec(dd.gcomm,lsize,dd.xm*dd.ym,localRHS,&lgRHS);CHKERRQ(ierr);
  
  /* KSPFETI */
  ierr = KSPCreate(dd.gcomm,&ksp_feti);CHKERRQ(ierr);
  ierr = KSPSetType(ksp_feti,KSPFETI);CHKERRQ(ierr);
  ierr = KSPSetOperators(ksp_feti,lgmat,lgmat);CHKERRQ(ierr);
  ...
  if (PetscNot(take_times)) {
    sprintf(nameFileIts,"iterations%03i.log",sizeG);
    ierr = KSPSetMonitorASCIIFile(ksp_interface,nameFileIts);CHKERRQ(ierr);
  }
  /* SetUp and Solve*/
  ...
  ierr = VecDestroy(&lgRHS);CHKERRQ(ierr);
  ierr = VecDestroy(&localRHS);CHKERRQ(ierr);
  ierr = KSPDestroy(&ksp_feti);CHKERRQ(ierr);
