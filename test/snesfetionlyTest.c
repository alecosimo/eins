static char help[] = "Solve the 2D Poisson equation using Finite Differences 
in a cavity of lx*ly. The FETI solver is used from the SNES interface 
specifying SNESFETIONLY\n\n\ 
Exaple usage:\n\ mpiexec -n 4 ./poissonFD -npx 2
-npy 2 -nex 2 -ney 2\n\ Dirichlet boundaries on x=0 side by
default. Options:\n\ -boundary: Dirichlet BC value\n\ -source: source
value\n\ -npx,npy: subdomains per dimension\n\ -nex,ney: number of
divisions per dimension\n\ -lx,ly: length of the cavity in the x and y
directions\n\n";
...
int main(int argc,char **args)
{
  ...
  ierr = SNESCreate(dd.gcomm,&snes);CHKERRQ(ierr);
  ierr = SNESSetType(snes,SNESFETIONLY);CHKERRQ(ierr);
  
  /* Setting FETI */
  ierr = SNESGetFETIContext(snes,&feti);CHKERRQ(ierr);
  ...
  /* Solve SNES problem using FETIONLY */
  ierr = VecSet(u_local,0);CHKERRQ(ierr);
  ierr = SNESSolve(feti,NULL,u_local);CHKERRQ(ierr);
